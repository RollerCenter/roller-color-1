...
// ============================================================
// APPEL JSONP (contourne le blocage CORS de Google)
// ============================================================
function jsonpCall(url) {
  return new Promise((resolve, reject) => {
    const cbName = 'rc1cb_' + Math.random().toString(36).slice(2);
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    const data = await jsonpCall(`${SCRIPT_URL}?action=check&code=${encodeURIComponent(code)}`);
...
async function generateTicket() {
  ...
  try {
    const url = `${SCRIPT_URL}?action=register`
      ...
      + `&prenom=${encodeURIComponent(data.prenom)}`
      + `&email=${encodeURIComponent(data.email)}`
      + `&whatsapp=${encodeURIComponent(data.whatsapp)}`
      + `&taille=${encodeURIComponent(data.taille)}`
      + `&niveau=${encodeURIComponent(data.niveau)}`
      + `&statut=${encodeURIComponent(data.statut)}`
      + `&prix=${encodeURIComponent(data.prix)}`
      + `&pointure=${encodeURIComponent(data.pointure || '-')}`
      + `&ref=${encodeURIComponent(ref)}`;

    const result = await jsonpCall(url);

    if (!result.success) {
      if (result.reason === 'already_used') {
        showAlreadyUsed();
      } else {
        showToast('❌ Code invalide. Contactez-nous.');
      }
    ...
    new QRCode(qrContainer, {
      text: JSON.stringify({ ref, code: currentAccessCode, nom: `${data.prenom} ${data.nom}`, taille: data.taille, event: 'ROLLER COLOR 1', date: '27 Juin 2025' }),
